    ...
    def load_players(self, ranking_type='expert'):
        ...
        try:
            ...
            with open(rankings_path, 'r', encoding='utf-8') as file:
                ...
                for row in reader:
                    try:
                        ...
                        if ranking_type == 'ml':
                            ...
                            # Extract base position from POS field for expert rankings
                            full_pos = row.get('POS', '')
                            player_pos = ''.join(c for c in full_pos if not c.isdigit())
                        
                        # Use Rank for expert rankings, Predicted_FP for ML rankings
                        if ranking_type == 'ml':
                            try:
                                points = float(row.get('Predicted_FP', 0))
                                if points < 0:  # Skip negative predictions
                                    continue
                                rank_value = points
                            except ValueError:
                                continue
                        ...
                        seen_players.add(player_name.lower())  # Add to seen players
                ...
                # Sort players
                if ranking_type == 'ml':
                    # Sort by predicted points in descending order (highest first)
                    self.available_players.sort(key=lambda x: float(x.avg_rank), reverse=True)
                else:
                    # Sort by rank in ascending order (lowest first)
                    self.available_players.sort(key=lambda x: float(x.avg_rank))
...
def main():
    ...
    if st.session_state.page == "Setup":
        ...
        with col2:
            # Create a row for the header and switch button
            ...
            with header_col2:
                if st.button(
                    "↺",
                    help="Switch between Expert and ML Rankings",
                    type="secondary",
                    key="switch_rankings"
                ):
                    ...
                    if success:
                        st.success(f"Using {'ML' if st.session_state.using_ml else 'Expert'} Rankings")
                        st.rerun()
                    else:
            ...
            if position == 'Favorites':
                ...
                if favorite_players:
                    for player in favorite_players:
                        ...
                        with col2:
                            if st.button(f"Draft", key=f"draft_fav_{player.name}_{player.avg_rank}"):
